...
class FrameHeader: 
    ...
    def __init__(self, Mode: int, GroupId: int, InGroupId: int, PayloadLen: int, IsLast: bool): 
        ...
        :param PayloadLen: 有效负载长度 (12bit) - 解析时根据此长度剔除末尾随机填充
    ...
    def ToBits(self) -> List[int]: 
        ...
        打包顺序：Sync(8) + Mode(2) + GroupId(8) + InGroupId(4) + PayloadLen(12) + IsLast(1) + Reserved(5) + CRC8(8) = 48 bits
        """ 
        ...
        CombinedValue = (self.SYNC_BYTE << 32) | \
                        ...
                        ((self.PayloadLen & 0xFFF) << 6) | \
                        (self.IsLast << 5) 
        # 末位留白 5 bit 作为预留 (Reserved)
        ...
        # 4. 转化为 Bit 列表返回 (填充至 48 位防止高位丢失)
        return [int(Bit) for Bit in bin(FinalValue)[2:].zfill(48)] 
    ...
    def FromBits(cls, BitList: List[int]) -> Optional['FrameHeader']: 
        ...
        return cls( 
            ...
            PayloadLen = (RawValue >> 6) & 0xFFF, 
            IsLast = bool((RawValue >> 5) & 0x01) 
        ) 
