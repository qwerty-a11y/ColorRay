...
def matrix_to_colors(
    ...
    patch_size: int = Config.DataBlocks,                    # 预期提取的颜色列表长度，与 common.Config 一致
    ...
    :param patch_size: 预期提取的颜色列表长度（须与 common.Config.DataBlocks 一致）
    :param start_pos: 填充时的起始一维索引（需与fill_matrix的start_pos一致）
    :param step: 填充时使用的互质步长（需与fill_matrix的step一致）
    :return: 还原后的原始颜色列表（仅包含填充的 patch_size 个颜色，无基础结构色）
